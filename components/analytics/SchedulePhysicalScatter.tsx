...
interface SchedulePhysicalScatterProps {
...
export function SchedulePhysicalScatter({ points }: SchedulePhysicalScatterProps) {
  ...
  // Count points where schedule elapsed > physical progress (schedule gap > 0)
  const delayedCount = points.filter((p) => p.scheduleProgressGap > 0).length
  const delayedPct = points.length > 0 ? ((delayedCount / points.length) * 100).toFixed(1) : '0'
    ...
    <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-xs space-y-4">
      ...
      <div className="bg-amber-50/70 border border-amber-200 rounded-xl p-3.5 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
        <div className="space-y-0.5">
          <div className="font-bold text-amber-950 flex items-center gap-1.5">
            ...
            <span>
              {delayedCount} projects ({delayedPct}%) are currently behind their planned delivery trajectory.
            </span>
          </div>
          <p className="text-amber-800 text-[11px]">
            This means elapsed schedule duration is outpacing on-ground physical completion for these projects.
          </p>
        </div>
      </div>
      ...
      <div className="relative w-full bg-slate-50/50 rounded-xl border border-slate-200 p-2 sm:p-3">
        ...
        >
          ...
          <polygon
            points={`${getX(0)},${getY(0)} ${getX(100)},${getY(0)} ${getX(100)},${getY(100)}`}
            fill="#fffbeb"
            fillOpacity="0.55"
          />
          ...
          >
            Parity line: elapsed time equals progress
          </text>
